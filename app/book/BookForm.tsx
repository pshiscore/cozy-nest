...
const labelClass = "block text-xs tracking-widest uppercase text-[#7a9478] mb-2";
const inputClass =
  ...
  return (
    <div className="bg-[#e8e2d9] rounded-2xl p-8 md:p-10">
      ...
      <form onSubmit={handleSubmit} noValidate className="space-y-5">
        ...
        {/* Neighborhood */}
        <div>
          <label className={labelClass} style={ff}>Neighborhood</label>
          <input
            name="neighborhood"
            type="text"
        ...
        <div>
          ...
          <div className="grid sm:grid-cols-2 gap-2">
            {(Object.entries(ADDONS) as [AddonValue, { label: string; duration: string }][]).map(
              ([key, { label, duration }]) => {
                  ...
                  >
                    ...
                    <span
                      className={`w-4 h-4 rounded shrink-0 flex items-center justify-center border transition-colors ${
                        checked ? "bg-[#3d5c44] border-[#3d5c44]" : "bg-[#f0ece4] border-[#b8a898]"
                      }`}
                    >
                      {checked && (
                    ...
                    <span>
                      <span className="text-sm text-[#3d5c44]" style={ff}>{label}</span>
                      <span className="text-xs text-[#7a9478] ml-1.5" style={ff}>· {duration}</span>
                    </span>
                  </label>
                );
              }
        ...
        {/* Message */}
        <div>
          <label className={labelClass} style={ff}>Anything we should know?</label>
          <textarea
            name="message"
            rows={4}
